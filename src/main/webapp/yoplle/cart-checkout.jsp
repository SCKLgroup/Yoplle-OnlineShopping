	...
	<section class="checkout spad">
		<div class="container">
			...
			<div class="checkout__form">
				<h4>주문 내역</h4>
						<form id="checkSuc" action="checkSucCart.do" method="POST" >
						<input name="order_dv_sts" type="text" hidden="hidden" value="주문확인">
					<div class="row">
						<div class="col-lg-8 col-md-6">
						<c:forEach var="i" items="${it}" varStatus="idx" begin="0" end="0">
							<div class="checkout__input">
								<p>
									주문자<span>*</span><span id="nameCheck"></span>
									<div id="nameError"></div>
								</p>
								<input type="text" name="order_rec_name" value="${i.user_name}">
								<input type="text" name="user_no" hidden="hidden" value="${i.user_no}">
							</div>
							<div class="checkout__input">
								<p>
									전화번호<span>*</span><span id="telCheck"></span>
								</p>
								<input type="text" name="order_rec_tel" value="${i.user_tel}">
							</div>
							<div class="checkout__input">
								<p>
									배송지<span>*</span>
									<button type="button" class="jaeil-site-btn" id="zip-set">배송지 선택</button>
								</p>
							...
							<div class="checkout__input">
								<p>이메일</p>
								<input type="text" value="${i.user_mail}">
							</div>
							</c:forEach>
						...
						<div class="col-lg-4 col-md-6">
							<div class="checkout__order">
								...
								<ul>
								<c:forEach var="i" items="${it}" varStatus="idx">
									<input type="text" value="${i.item_no}" hidden="hidden" name="itemno">
								</c:forEach>
								<c:set var="sum" value="0"/>
								<c:forEach var="i" items="${it}" varStatus="idx">
									<li><div id="nameTag">[${i.item_category}] ${i.item_name}</div> <span id="prices">${i.item_price*i.cart_quan} 원</span></li>
									<input type="text" name="item_noV" hidden="hidden" value="${i.item_no}">
									<input type="text" name="or_de_quanV" hidden="hidden" value="${i.cart_quan}">
									<input type="text" name="or_de_priceV" hidden="hidden" value="${i.item_price*i.cart_quan}">
									<c:set var="sum" value="${sum+(i.item_price*i.cart_quan)}"></c:set>
								</c:forEach>
								</ul>
								
								<div class="checkout__order__subtotal">
								
									상품 총 금액 <span><c:out value="${sum}"/> 원</span>
									<input type="text" hidden="hidden" name="or_de_price_all" value="${sum}">
								</div>
								<div class="checkout__order__send">
									배송 금액 <span id="dvPrice">3,000원</span>
								</div>
								<div class="checkout__order__total">
									총 금액 <span><c:out value="${sum+3000}"/> 원</span>
								</div>
								<div class="checkout__input__checkbox">
